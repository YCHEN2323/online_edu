    ...
    <c:forEach items="${sessionScope.classInFo}" var="info">
        <a>所在班级：${info.classname} &nbsp &nbsp &nbsp <br>课程号：${info.courseid}<br>
            ...
            任课教师：${sessionScope.teacher.name}<br><br><br>
                &lt;%&ndash;            <form action="/course/delete/${name.id}" method="post">&ndash;%&gt;
                &lt;%&ndash;                <input type="hidden" name="classlistid" value="${name.id}">&ndash;%&gt;
                &lt;%&ndash;                <input type="submit" value="删除课程">&ndash;%&gt;
                &lt;%&ndash;            </form>&ndash;%&gt;
        </a>
    </c:forEach>
</div>--%>
